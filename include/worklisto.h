...
struct Worklist {
	...
	Worklist(size_t max_size) {
		...
		cl_mem d_queue = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int) * max_size, NULL, NULL);
		cl_mem d_size = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int), NULL, NULL);
		cl_mem d_index = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int), NULL, NULL);
		clEnqueueWriteBuffer(queue, d_size, CL_TRUE, 0, sizeof(int), &max_size, 0, NULL, NULL);
		clEnqueueWriteBuffer(queue, d_index, CL_TRUE, 0, sizeof(zero), &zero, 0, NULL, NULL);
	}
	...
	void reset() {
		...
		clEnqueueWriteBuffer(queue, d_index, CL_TRUE, 0, sizeof(int), &zero, 0, NULL, NULL);
	}

	int nitems() {
		int index;
		//CUDA_SAFE_CALL(cudaMemcpy(&index, (void *)d_index, sizeof(int), cudaMemcpyDeviceToHost));
		clEnqueueReadBuffer(queue, d_index, CL_TRUE, 0, sizeof(int), &index, 0, NULL, NULL);
		return index;
	}
...
struct Worklist2: public Worklist {
	...
	/*__kernel int pop_id(int id, int &item) {
		if(id < *d_index) {
