...
void SpmvVerifier(int num_rows, int nnz, int *h_Ap, int *h_Aj, ValueType *h_Ax, ValueType *h_x, ValueType *h_y, ValueType *test_y) {
	printf("Verifying...\n");
	int *Ap = (int *)malloc((num_rows+1) * sizeof(int));
	for(int i = 0; i < num_rows + 1; i ++) Ap[i] = h_Ap[i];
	int *Aj = (int *)malloc(nnz * sizeof(int));
