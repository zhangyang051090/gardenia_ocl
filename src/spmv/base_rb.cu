...
__global__ void spmv_csr_scalar_kernel(const int num_rows, const int * Ap,  const int * Aj,
		const ValueType * Ax, const ValueType * x, ValueType * y, int ** value, 
...
*/
void SpmvSolver(int num_rows, int nnz, int *h_Ap, int *h_Aj, ValueType *h_Ax, ValueType *h_x, ValueType *h_y, int **h_value, int *h_block, int *h_row_start, int num_block_all) { 
	//print_device_info(0);
	int *d_Ap, *d_Aj;
	...
	int **d_value, *d_block, *d_row_start;
	CUDA_SAFE_CALL(cudaMalloc((void ***)&d_value, num_block_all * 4 * sizeof(int)));
	...
	CUDA_SAFE_CALL(cudaMemcpy(d_value, h_value, num_block_all * 4 * sizeof(int), cudaMemcpyHostToDevice));
