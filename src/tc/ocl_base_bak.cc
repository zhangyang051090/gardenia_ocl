...
"__kernel void tc_kernel(int m, __global int * row_offsets, __global int * column_indices, __global int * degree,__global int * total_sum,		\n" \
" __global int * total) 	      														 	\n" \
"{																			\n" \
...
"		}												  					\n" \
"														  					\n" \
"	total_sum[src] = total_num;									  						\n" \
"	}													  					\n" \
"														  					\n" \
"	//if(src==0){												  					\n" \
"	//for(int i = 1; i < 101; i ++)										  					\n" \
"	//	total_sum[i-1] = i;}										  					\n" \
"//	for(int s = global_size >> 1; s > 0; s >>= 1){								  					\n" \
"	//for(int s = 100 >> 1; s > 0; s >>= 1){								  					\n" \
"	//	if(src < s){											  					\n" \
"	//		total_sum[src] += total_sum[src + s];							  					\n" \
"	//	}												  					\n" \
...
"	/*if(src == 0){																	\n" \
"		int n = 0;											  					\n" \
"		while(global_size > (2<<n)){									  					\n" \
"			n++;											  					\n" \
"		}												  					\n" \
"														  					\n" \
"		//for(int i = global_size; i < (2<<n); i++)							  					\n" \
"		//	total_sum[i] = 0;									  					\n" \
"														  					\n" \
"														  					\n" \
"	int global_ext;										  					\n" \
...
"														  					\n" \
"														  					\n" \
"	    for(int s = global_ext >> 1; s > 0; s >>= 1){								  					\n" \
"		for(int j=0; j < s; j++){									  					\n" \
...
void TCSolver(int m, int nnz, int *row_offsets, int *column_indices, int *degree, int * total) {
	...
	int bound_num = 0;	
	while(globalSize > pow(2,bound_num)){
		bound_num ++;
	//printf("\nbound_num in is:\t%d\n\n", bound_num);	
	...
	unsigned int upper = pow(2,bound_num);
	...
	err |= clSetKernelArg(kernel, 4, sizeof(cl_mem), &d_total_sum);
	err |= clSetKernelArg(kernel, 5, sizeof(cl_mem), &d_total);
